...
function SS_values(p::Par)
    ...
    l_st = 0.4 # Labor
    k_st = gam*l_st # Capital
    y_st = z_bar*(k_st^α)*(l_st^(1-α)) # Output
    c_st = y_st-δ*k_st # Consumption
    w_st = (1-α)*z_bar*(k_st^α)*(l_st^(-α)) # Wage = marginal product of labor
    r_st = α*z_bar*(k_st^(α-1))*(l_st^(1-α)) # Rental rate of capital = marginal product of capital
    return k_st,y_st,c_st,r_st,w_st,l_st
end
# Test steady state function
k_st,y_st,c_st,r_st,w_st,l_st = SS_values(p)
println(" ")
println("------------------------")
println(" Steady State variables")
println("   Quantities: k = $k_st; y = $y_st; c = $c_st;")
println("   Prices:     r = $r_st; w = $w_st;")
println("------------------------")
println(" ")
# Get χ such that steady state labor = 0.4
function get_chi(p::Par,l_st,c_st,k_st)
    @unpack z_bar, α, β, δ, σ, η = p
    chi = (c_st^(-σ))*z_bar*(1-α)*(k_st^α)*(l_st^(-α-η))
    return chi
end
global χ = get_chi(p,l_st,c_st,k_st)
